...
def main(args):
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, load_in_4bit=True, device_map='auto')
    if args.load_lora:
        model = PeftModel.from_pretrained(model, args.lora_name_or_path, load_in_4bit=True, device_map='auto')
    model.eval()
    ...
    def compute_prob(inp, contxt_len, answer_tokens):
        ...
        token_log_probs = torch.gather(
            vocab_log_probs, dim=2, index=answer_tokens[:, 2:, None]
