...
def main(
    ...
    if load_lora:   
        model = PeftModel.from_pretrained(model, lora_name_or_path, device_map='auto')
    model.eval()
    ...
    # with open("/mnt/ssd3/chunhui/elsie/world-model-for-language-model/test.log", "w") as f:
    #     for i in range(len(eval_preds)):
    #         f.write(eval_preds[i] + "\n\n\n")
